...
import { ArrowLeft, Users, Gift, TrendingUp, Star, Loader2 } from "lucide-react";
...
const PartnerScreen = ({ onBack, onRequireAuth }: { onBack: () => void; onRequireAuth?: () => void }) => {
  ...
  const fetchReferralData = useCallback(async () => {
    ...
    try {
      ...
      if (data.data) {
        setReferralData(normalizeReferralData(data.data));
      }
    } catch (error) {
        ...
        <AnimatePresence mode="wait">
          {!isAuthenticated ? (
            ...
            <motion.div key="dashboard" initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }}>
              <ReferralDashboard data={referralData} error={loadError} onRetry={fetchReferralData} />
            </motion.div>
          )}
        </AnimatePresence>
      )}
